      ...
      * configure output file
           select grad-file
               assign to "../../../Data/lab7_grad.out"
               organization is line sequential.

           select nongrad-file
               assign to "../../../Data/lab7_nongrad.out"
               organization is line sequential.
       data division.
       file section.
      ...
      * declare an input record definition 
       ...
       01 input-line.
           ...
           05 il-budget-est            pic 9(6)V99.

       fd grad-file
           data record is grad-line
           record contains 120 characters.
       
       01 G-output-line                pic x(120).

       fd nongrad-file
           data record is nongrad-line
           record contains 120 characters.
           
       01 N-output-line             pic x(120).
      ...
      * Displays the file's title and page number for grads
       01 ws-G-rpt-heading.
           05 filler                   pic x(18)
              value spaces.
           05 filler                   pic x(22)
               value "GRADUATE SALARY REPORT".
           05 filler                   pic x(26)
               value spaces.
           05 filler                   pic x(6)
               value "PAGE  ".
           05 ws-G-page-count            pic 9
               value 1.

      * Displays the file's title and page number for non-grads
       01 ws-N-rpt-heading.
           05 filler                   pic x(18)
              value spaces.
           05 filler                   pic x(26)
               value "NON-GRADUATE SALARY REPORT".
           05 filler                   pic x(26)
               value spaces.
           05 filler                   pic x(6)
               value "PAGE  ".
           05 ws-N-page-count            pic 9
               value 1.
      * First heading line
       01 ws-heading-line1.
           05  filler                  pic x(3)
      ...
      * Don't need this for this lab (I think, like 90% sure)
       01 ws-sub-emp-l1.
           05  filler                  pic x(15)
      ...
      *Constants
       ...
       procedure division.
           open input input-file,
               output grad-file, nongrad-file.

           perform 100-headline-check.
           read input-file
      ...
      * For every 10th employee it will print the headings and subtotals
      ...
      *    perform 500-subtotals-and-average-pay.
           close input-file grad-file nongrad-file.
           goback.

       100-headline-check.
           if (il-edu-code = ws-GRADS)
               perform 100-grad-heading
           end-if.
           
           if (il-edu-code = ws-NON-GRADS)
               perform 110-non-grad-heading
           end-if.
...
.          write N-output-line         from ws-heading-line2.
       ...
       300-G-subtotals.
           write G-output-line         from ws-sub-emp-l1.
           ...
           write G-output-line         from ws-sub-emp-l2.
           write G-output-line         from ws-new-line.

       310-N-subtotals.
           write N-output-line         from ws-sub-emp-l1.
           move ws-analyst-stotal      to ws-analyst-final.
           move ws-sprog-stotal        to ws-sprog-final.
           move ws-prog-stotal         to ws-prog-final.
           move ws-jrprog-stotal       to ws-jrprog-final.
           move ws-unclass-stotal      to ws-unclass-final.
           write N-output-line         from ws-sub-emp-l2.
           write N-output-line         from ws-new-line.
      ...
      *    write output-line           from ws-avg-total-line2.
       ...
       510-test1.
           if (ws-line-count >= ws-emp-limit)
               add ws-inc-one-cnst to ws-G-page-count
               perform 300-G-subtotals
               perform 320-G-new-page-head
       ...
       520-test2.
           if (ws-line-count >= ws-emp-limit)
               add ws-inc-one-cnst to ws-N-page-count
               perform 310-N-subtotals
               perform 330-N-new-page-head
               perform 400-reset-count-and-total
           else
               add ws-inc-one-cnst to ws-line-count
           end-if.
       ...
       500-test3.
           if (il-edu-code = ws-GRADS)
               perform 510-G-output
           end-if.
           
           if (il-edu-code = ws-NON-GRADS)
               perform 520-N-output
           end-if.
       510-G-output.
